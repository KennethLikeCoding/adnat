...
export class DashboardComponent extends React.Component {
                ...
                resp => {
                    ...
                    if (orgId) {
                        axios.get(env.BASE_URL + 'organisations/'+orgId, headers).then(
                            resp => {
                                let org = resp.data;
                                this.setState({orgId: org.id, orgName: org.name, rate: org.hourlyRate});
                            }
                        )
                    }
            })
    ...
    handleOrgChange = (org) => {
        if (org) {
            ...
            this.setState({orgId: null, orgName: null, rate: null});
        }        
    }
    ...
    handleUserChange = (user) => {
        this.setState({username: user.name, email: user.email});
    }
    ...
    renderEditUser() {
        if (this.state.editUser) {
            ...
            return (<EditUserComponent user={user} onUserChange={this.handleUserChange} onExit={()=>this.setState({editUser: false})} headers={this.getHeaders()}/>)
        }
    }
            ...
            return (
                <div className="ui container pt--2">
                    ...
                    <p>
                        ...
                        {(!this.state.editUser) && (<span className="ml--2"><button onClick={()=>this.setState({editUser: true})} className="ui blue basic mini button">Edit</button></span>)}
